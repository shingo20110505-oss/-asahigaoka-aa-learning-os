...
const STATUS=window.AA_ANSWER_FEEDBACK_AUDIO={version:'1.0.1',installed:false,correctPlays:0,wrongPlays:0,lastError:null};window.AA_EXERCISE_ANSWER_SOUND=STATUS;
let audioCtx=null;
...
function isTestLike(q){try{const kind=String(state?.session?.kind||'').toLowerCase();return q?.testMode===true||state?.session?.testMode===true||/(simulator|exam|mock|aichi.*test|practice-test)/.test(kind)}catch(_){return false}}
...
function install(attempt=0){if(typeof selectAnswer!=='function'||typeof playSuccessCue!=='function'||typeof state==='undefined'){if(attempt<100)setTimeout(()=>install(attempt+1),50);return}
 if(selectAnswer.__aaAnswerAudioPatched||selectAnswer.__aaExerciseAnswerSoundPatched){STATUS.installed=true;return}
 playSuccessCue=function(){return correctCue(currentQuestionSafe())};
 const before=selectAnswer;selectAnswer=function(idx){const q=currentQuestionSafe(),hadFeedback=!!state?.session?.feedback,correct=!!q&&Number(idx)===Number(q.answerIndex);const out=before(idx);if(!hadFeedback&&!correct)wrongCue(q);return out};selectAnswer.__aaAnswerAudioPatched=true;selectAnswer.__aaExerciseAnswerSoundPatched=true;
 patchSettings();STATUS.installed=true;STATUS.playCorrect=correctCue;STATUS.playWrong=wrongCue;
