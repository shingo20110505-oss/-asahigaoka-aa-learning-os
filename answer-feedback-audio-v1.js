(()=>{'use strict';
if(window.__AA_ANSWER_FEEDBACK_AUDIO_V39__)return;
window.__AA_ANSWER_FEEDBACK_AUDIO_V39__=true;
...
// iPhone timing fix: prepare a fresh detached Audio on answer pointerdown and play that exact instance on pointerup.
// This avoids the repeated currentTime=0 seek/restart path that can delay the second and later questions in iOS WebKit.

const PREF_KEY='aa-answer-feedback-audio-v3';
const STATUS=window.AA_ANSWER_FEEDBACK_AUDIO={version:'3.0.1',build:'premium-kira-tapguard-v39-fresh-audio',installed:false,enabled:true,backend:'htmlaudio',correctPlays:0,wrongPlays:0,lastError:null,lastCue:null,legacyCoreSuppressed:false,layoutSafe:true,soundSet:'premium-kira-original',trigger:'tap-release-fresh-audio'};
...
// Preserve the iPhone tap-release playback path. WAV data is prepared during idle time.
// A fresh Audio instance is created on pointerdown for the touched answer and played on pointerup.
...
function audioFor(correct){
 ...
 media[key]=createAudio(correct);
 return media[key];
}
function freshAudio(correct){
 const a=createAudio(correct);
 if(!a)return audioFor(correct);
 try{a.load?.()}catch(_){}
 return a;
}
...
function playCue(correct,prepared=null){
 if(!STATUS.enabled)return false;
 const a=prepared||audioFor(correct);if(!a)return false;
 try{
  if(current&&current!==a&&!current.ended){current.pause();try{current.currentTime=0}catch(_){}}
  current=a;a.muted=false;a.volume=.9;
  // Fresh pointerdown-prepared instances start at zero already; fallback instances are reset for keyboard/programmatic paths.
  if(!prepared)try{a.currentTime=0}catch(_){}
  const p=a.play();
...
function triggerInfo(info,prepared=null){
 ...
 return playCue(!!info.correct,prepared);
}
...
// Prepare a fresh detached Audio there, then play that same instance on pointerup only if movement stayed tap-sized.
document.addEventListener('pointerdown',e=>{
 const el=answerElement(e);if(!el)return clearTap();
 const info=answerInfo(el);if(!info)return clearTap();
 tap={id:e.pointerId,x:e.clientX,y:e.clientY,el,info,moved:false,audio:freshAudio(!!info.correct)};
},true);
