...
function syncBrand(){const app=document.getElementById('app');if(!app)return false;let changed=false,top=app.querySelector(':scope > .top');if(!top){top=document.createElement('header');top.className='top';top.innerHTML='<div class="toprow"><div class="brand"><div class="mark">R</div><div><h1>Rise</h1><p>旭丘合格のための学習OS</p></div></div><div class="topBtns"></div></div>';app.prepend(top);changed=true}let row=top.querySelector('.toprow');if(!row){row=document.createElement('div');row.className='toprow';top.appendChild(row);changed=true}let brand=row.querySelector('.brand');if(!brand){brand=document.createElement('div');brand.className='brand';brand.innerHTML='<div class="mark">R</div><div><h1>Rise</h1><p>旭丘合格のための学習OS</p></div></div>';row.prepend(brand);changed=true}if(!row.querySelector('.topBtns')){const buttons=document.createElement('div');buttons.className='topBtns';row.appendChild(buttons);changed=true}const mark=brand.querySelector('.mark'),title=brand.querySelector('h1'),subtitle=brand.querySelector('p');if(mark&&mark.textContent!=='R'){mark.textContent='R';changed=true}if(title&&title.textContent.trim()!=='Rise'){title.textContent='Rise';changed=true}if(subtitle&&subtitle.textContent!=='旭丘合格のための学習OS'){subtitle.textContent='旭丘合格のための学習OS';changed=true}return changed}
