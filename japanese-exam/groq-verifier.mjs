...
function assertMajor(major) {
  ...
  if (!VALID_MAJORS.includes(n)) {
    throw new JapaneseVerificationError('invalid_japanese_major', '国語の大問番号が正しくありません。', 400);
  }
  return n;
}
...
function ensureValidPack(pack) {
  ...
  if (!structural.ok) {
    throw new JapaneseVerificationError(
      'japanese_structure_rejected',
      '国語問題の決定的検証を通過していません。',
      422,
      structural.errors.slice(0, 10).join('|')
    );
  }
  return pack;
}
...
export function buildJapaneseBlindChunk(pack, major) {
  ...
  if (!questions.length) {
    throw new JapaneseVerificationError('japanese_major_empty', '検証対象の国語問題がありません。', 400);
  }
...
export function verifyJapaneseChunkAgreement(pack, major, result, threshold = confidenceThreshold) {
  ...
  if (result?.pass !== true || !Array.isArray(result?.answers) || result.answers.length !== expected.length) {
    return { ok: false, errors: ['verification_not_passed'] };
  }

  const seen = new Set();
  for (const { question, questionIndex } of expected) {
    const answer = result.answers.find(item => item?.questionIndex === questionIndex);
    if (!answer || seen.has(questionIndex)) {
      errors.push(`missing_or_duplicate:${question.id}`);
      continue;
    }
    ...
    if (structured) {
      ...
      if (!Array.isArray(answer.markChoiceIndexes) || answer.markChoiceIndexes.length !== question.marks.length) {
        errors.push(`mark_count:${question.id}`);
      } else {
      ...
      if (new Set(indexes).size !== indexes.length || indexes.some(index => !Number.isInteger(index) || !question.choices[index])) {
        errors.push(`answer_indexes:${question.id}`);
      } else {
      ...
      if (!Array.isArray(answer.choiceRelations) || answer.choiceRelations.length !== question.choices.length) {
        errors.push(`relation_count:${question.id}`);
      } else if (question.choices.some((choice, index) => answer.choiceRelations[index] !== choice.relation)) {
        errors.push(`choice_disagreement:${question.id}`);
      }
    }

    if (selectedMajor !== 2) {
      if (!Array.isArray(answer.evidence) || answer.evidence.length < 1) {
        errors.push(`missing_evidence:${question.id}`);
      } else {
        for (const evidence of answer.evidence) {
          const passage = visiblePassages[evidence?.passageIndex];
          const paragraph = passage?.paragraphs?.[Number(evidence?.paragraph) - 1];
          if (!passage || typeof evidence?.quote !== 'string' || evidence.quote.length < 4 || !paragraph?.includes(evidence.quote)) {
            errors.push(`evidence_quote:${question.id}`);
          }
        }
      }
    }
...
export async function verifyJapaneseMajorWithGroq(env, pack, major) {
  ...
  if (!agreement.ok) {
    throw new JapaneseVerificationError(
      ...
      422,
      agreement.errors.slice(0, 12).join('|')
    );
  }
...
export async function verifyJapanesePackWithGroq(env, pack, {
  cooldownMs = 61000,
