...
export function passagePrompt(repair=''){return `${SPEC}\n今回は本文だけを生成する段階。次の4件を必ず1件ずつ、majorは文字列ではなく整数で生成する。\n1. id="major1-main", major=1, role="main", genre="論説"\n2. id="major1-reference", major=1, role="reference", genre="参考文"\n3. id="major3-main", major=3, role="main", genre="小説"または"随筆"\n4. id="major4-main", major=4, role="main", genreは"古文（創作教材）"または"漢文風書き下し文（創作教材）"\nJSON {passages:[{id,major,role,title,genre,rights:{kind:"original",label:"本アプリ作成オリジナル"},paragraphs:[...] }], propositions:[{sourceId,paragraph,subject,predicate,scope,causality,modality}]} を返す。passagesは上記の正確な4件で、省略・統合・追加をしない。各paragraphsは文字列配列。論説と文学本文は各1000字以上、参考文150字以上、注釈付き古典200字以上。設問・正解・選択肢はまだ作らない。${repair?`\n前回は構造検査で不合格だった。本文を使い回さず、次の診断だけを直して4件を作り直す: ${String(repair).slice(0,220)}`:''}`;}
