...
const VERSION = '2.2.9';
const CACHE_NAME = `asahigaoka-aa-os-${VERSION}-companion750-loginzip1-analytics1-prodtest1-examples2-quality1-readingnatural3-settingsguard2-chrono1000-v24content1-quotes1-storage1-difficulty2-challenge1-glosstap1-answeraudio34-chronodeep3-chronocurated500-v1`;
...
self.addEventListener('fetch',event=>{const request=event.request;if(request.method!=='GET')return;const url=new URL(request.url);if(url.origin!==self.location.origin)return;if(request.mode==='navigate'){event.respondWith((async()=>{try{const response=await fetch(request,{cache:'no-cache'});if(response.ok){const cache=await caches.open(CACHE_NAME);await cache.put(request,response.clone())}return response}catch(_){return(await caches.match(request))||(await caches.match(APP_URL))||(await caches.match(OFFLINE_URL))}})());return}if(!url.href.startsWith(BASE))return;const fresh=/\/(aa-companion-v2|storage-resilience-v1|reading-gloss-tap-v1|answer-feedback-audio-v1|aa-companion-mobile-fix|companion-settings-only-guard-v1|voice-selftest-v1|companion7-runtime|login-companion-v1|analytics-daily-v1|login-production-test-v1|curriculum-expansion-v24|source-quote-bank-v1|difficulty-challenge-v1|reading-natural-v2|reading-natural-v3|explanation-examples-v2|question-quality-v1|difficulty-engine-v1|companion7-check|chronologia-live-companion|chronologia-deep-explanations-v3|chronologia-deep-polish-v1|chronologia-curated-501-600|chronologia-curated-601-700|chronologia-curated-701-800|chronologia-curated-801-900|chronologia-curated-901-1000|chronologia-v7-runtime|chronologia-v7-data-1|chronologia-v7-data-2a|chronologia-v7-data-2b|chronologia-v7-data-3|chronologia-v7-data-4|chronologia-v7-overrides|v23-loader|v23-pet-settings|v23-compat-audit)\.js$/.test(url.pathname);event.respondWith((async()=>{if(fresh){try{const response=await fetch(request,{cache:'no-store'});if(response.ok){const cache=await caches.open(CACHE_NAME);await cache.put(request,response.clone())}return response}catch(_){return(await caches.match(request))||new Response('',{status:504,statusText:'Offline'})}}const cached=await caches.match(request);if(cached)return cached;try{const response=await fetch(request);if(response.ok){const cache=await caches.open(CACHE_NAME);await cache.put(request,response.clone())}return response}catch(_){return new Response('',{status:504,statusText:'Offline'})}})())});