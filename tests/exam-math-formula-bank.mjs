...
for(const file of ['formulas.js','practice.js','visuals.js']){
...
assert.ok(visuals,'visual formula bank missing');
...
assert.equal(visuals.ids.length,18,'expected 18 visualized formulas');
assert.equal(new Set(visuals.ids).size,visuals.ids.length,'duplicate visual formula id');
for(const id of visuals.ids){
  assert.ok(bank.items.some(x=>x.id===id),`visual references unknown formula ${id}`);
  const svg=visuals.render(id);
  assert.ok(svg.includes('<svg'),`${id}: visual must contain svg`);
  assert.ok(svg.includes('<figcaption>'),`${id}: visual must include a learning caption`);
}
for(const id of ['a01','a02','a03','a04','a05','a08','a22','a23']) assert.ok(visuals.ids.includes(id),`missing high-value visual ${id}`);
...
const formulaJs=fs.readFileSync(new URL('../high-school-math/formulas.js',import.meta.url),'utf8');
assert.ok(formulaJs.includes("./visuals.js?v=1.0.0"),'formula bank must load visual diagrams');
const visualJs=fs.readFileSync(new URL('../high-school-math/visuals.js',import.meta.url),'utf8');
for(const token of ['mathDiagram','diagramTag','図で見抜く','MutationObserver']) assert.ok(visualJs.includes(token),`visual UI missing ${token}`);
const sw=fs.readFileSync(new URL('../high-school-math/sw.js',import.meta.url),'utf8');
for(const asset of ['./index.html','./formulas.js','./practice.js','./visuals.js']) assert.ok(sw.includes(asset),`scoped SW missing ${asset}`);
assert.ok(sw.includes("rise-exam-formula-lab-1.3.0"),'scoped SW cache version must be 1.3.0');

console.log(`exam-math-formula-lab ok: formulas=${bank.items.length} speed=${speed.length} advanced=${advanced.length} drills=${practice.items.length} visuals=${visuals.ids.length}`);
