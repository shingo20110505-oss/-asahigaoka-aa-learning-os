...
import {freeGate,generatePack,normalizeJapaneseMaterial,validateJapaneseMaterial,validateLibrary} from '../scripts/japanese-library.mjs';
...
assert.throws(()=>freeGate({GEMINI_FREE_TIER_CONFIRMED:'true',GEMINI_API_KEY:'dummy',GEMINI_MODEL:'gemini-3.5-flash'}),/groq_key_missing/);checks++;
assert.throws(()=>freeGate({GEMINI_FREE_TIER_CONFIRMED:'true',GEMINI_API_KEY:'dummy',GEMINI_MODEL:'gemini-3.5-flash',GROQ_API_KEY:'dummy',GROQ_MODEL:'other-model'}),/unconfirmed_groq_model/);checks++;
...
const env={GEMINI_FREE_TIER_CONFIRMED:'true',GEMINI_API_KEY:'test-not-real',GEMINI_MODEL:'gemini-3.5-flash',GROQ_API_KEY:'test-groq-key',GROQ_MODEL:'openai/gpt-oss-20b'};
calls=0;await assert.rejects(generatePack({env,call:async()=>{calls++;const e=Error('quota');e.status=429;throw e;}}),/quota/);check(calls===1,'Gemini quota error has no retries');
...
const stringMajorMaterial=copy(material);stringMajorMaterial.passages=stringMajorMaterial.passages.map(p=>({...p,major:String(p.major)}));
const normalizedMaterial=normalizeJapaneseMaterial(stringMajorMaterial);
check(validateJapaneseMaterial(normalizedMaterial).ok&&normalizedMaterial.passages.every(p=>Number.isInteger(p.major)),'material normalizes numeric-string majors and infers required roles');
let verifierCalls=0;
const generated=await generatePack({env,call:async()=>{calls++;if(calls===1)return material;if(calls===2)return candidate;throw Error('unexpected_generation_call');},verify:async pack=>{verifierCalls++;return {verified:true,provider:'groq',model:'openai/gpt-oss-20b',majors:[1,2,3,4],questionCount:pack.questions.length};}});
check(calls===2&&verifierCalls===1&&generated.quality.verified&&validatePack(generated).ok,'Gemini generation plus injected Groq verification contract');
check(generated.quality.materialGenerationAttempts===1,'valid Japanese material uses one generation attempt');
check(generated.quality.generationProvider==='gemini'&&generated.quality.generationModel==='gemini-3.5-flash','Gemini generation metadata pinned');
check(generated.quality.verificationProvider==='groq'&&generated.quality.verificationModel==='openai/gpt-oss-20b','Groq verification metadata pinned');
check(generated.quality.verificationMethod==='cross-provider-blind-answer-check','cross-provider method recorded');
calls=0;verifierCalls=0;
const missingClassical={...copy(material),passages:copy(material.passages).filter(p=>p.major!==4)};
const repaired=await generatePack({env,call:async()=>{calls++;if(calls===1)return missingClassical;if(calls===2)return stringMajorMaterial;if(calls===3)return candidate;throw Error('unexpected_repair_call');},verify:async pack=>{verifierCalls++;return {verified:true,provider:'groq',model:'openai/gpt-oss-20b',majors:[1,2,3,4],questionCount:pack.questions.length};}});
check(calls===3&&verifierCalls===1&&repaired.quality.materialGenerationAttempts===2,'malformed material gets one bounded shape repair before full question generation');
...
check(!/generativelanguage|api\.openai|GEMINI_API_KEY|GROQ_API_KEY/.test(appCode),'browser has no AI endpoint or secret');
