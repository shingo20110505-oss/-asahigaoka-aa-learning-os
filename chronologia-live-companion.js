...
function forceTimelineReady(){const run=()=>{try{if(typeof updateStats==='function')updateStats();if(typeof renderTimeline==='function')renderTimeline();if(typeof renderSync==='function')renderSync();if(typeof renderWeak==='function')renderWeak();if(typeof DATA!=='undefined'){document.querySelectorAll('.chrono-v7-badge').forEach(b=>b.textContent=`多角史 ${DATA.length}件`);document.documentElement.dataset.chronologiaItems=String(DATA.length);document.dispatchEvent(new CustomEvent('chronologia:content-ready',{detail:{items:DATA.length}}))}}catch(e){console.warn('Chronologia first-render refresh failed',e)}};run();requestAnimationFrame(run);setTimeout(run,80);setTimeout(run,250);setTimeout(run,700)}
...
async function boot(){await ensureChronologia7();await ensureDeepExplanations();forceTimelineReady();ensure()}
