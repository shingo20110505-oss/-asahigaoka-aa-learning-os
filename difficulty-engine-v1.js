...
function rowScore(row,d,subject){
 ...
 let format=0;
 const text=`${row.prompt||''} ${row.explanation||''}`;
...
function chooseRow(subject,d,areas=null,exclude=new Set()){
 d=currentD(d);const pool=rowPool(subject,d,areas,exclude);if(!pool.length)return null;
 return pool.map(r=>({r,s:rowScore(r,d,subject)})).sort((a,b)=>b.s-a.s)[0].r;
}
function exactQuestion(subject,d,areas=null,exclude=new Set()){
 d=currentD(d);const row=chooseRow(subject,d,areas,exclude);if(!row)return null;
 const q=API.makeQuestion(row,false);if(!q)return null;
 q.requestedDifficulty=d;q.actualDifficulty=Number(row.difficulty)||6;q.difficulty=q.actualDifficulty;
 q.courseLevel=courseLevel(d);q.difficultyLabel=BAND_LABELS[d];
 q.source=Object.assign({},q.source||{},{area:row.area,difficulty:q.actualDifficulty,requestedDifficulty:d,difficultyEngine:'exact-1-11-v1'});
 q.reviewKey=q.reviewKey||`v2:${subject}:${row.id}`;
 return q;
}
const originals={
...
function vocabDifficulty(v){
 ...
 if(src==='aa24-curated')d=v?.pos==='phrase'?8:7;
 if(word.includes(' '))d++;if(word.length>=12)d++;return clampD(d);
}
...
if(originals.vocab)makeVocabQ=function(forced=null,forcedFormat=null){const d=currentD();const v=forced||vocabWords(1,d)[0];return originals.vocab(v,forcedFormat||vocabFormat(v,d,0))};
if(originals.planVocab)planVocabQueue=function(count=8){const d=currentD(),words=vocabWords(Math.min(count,(typeof vocabPool==='function'?vocabPool().length:DATA.vocab.length)),d);return words.map((v,i)=>originals.vocab(v,vocabFormat(v,d,i))).slice(0,count)};
...
if(originals.kanji)makeKanjiQ=function(forced=null,forcedFormat=null){const d=currentD(),k=forced||kanjiWords(1,d)[0];return originals.kanji(k,forcedFormat||kanjiFormat(k,d,0))};
...
function exactUnitQueue(config){
 ...
 for(let i=0;i<count;i++){
   ...
   exclude.add(String(q.reviewKey||q.id));q.examUnit=unit;q.testMode=false;q.points=1;q.context=`unit-practice-${subject}-${unit}`;out.push(q);
 }
 return out;
}
...
function audit(){
 ...
 return{version:'1.0.0',currentDifficulty:currentD(),bySubject,english,unitPracticeExact:['japanese','math','science','social'],pass};
}
window.AA_DIFFICULTY_ENGINE={version:'1.0.0',currentDifficulty:currentD,courseLevel,chooseRow,exactQuestion,vocabDifficulty,kanjiDifficulty,exactUnitQueue,startExactUnitPractice,audit};
try{window.AA_DIFFICULTY_AUDIT=audit()}catch(e){window.AA_DIFFICULTY_AUDIT={version:'1.0.0',pass:false,error:String(e?.message||e)}}
