...
for(const f of ['curriculum-v2-data.js','v23-japanese.js','v23-math.js','v23-science.js','v23-social.js','curriculum-expansion-v24.js','source-quote-bank-v1.js','difficulty-challenge-v1.js'])vm.runInContext(fs.readFileSync(f,'utf8'),ctx,{filename:f});
...
const result={version:E.version,challenge:ctx.AA_DIFFICULTY_CHALLENGE_STATS,runtimeAudit:ctx.AA_DIFFICULTY_AUDIT,subjects:{},english:{},unitPractice:{},failures:[]};
function mean(a){return a.reduce((s,x)=>s+x,0)/Math.max(1,a.length)}
for(const s of subs){const levels={};for(const d of [1,3,5,7,9,11]){const vals=[];for(let i=0;i<24;i++){const r=E.chooseRow(s,d);if(r)vals.push(Number(r.difficulty)||6)}levels[d]=+mean(vals).toFixed(2);if(!vals.length||Math.abs(levels[d]-d)>2.6)result.failures.push(`${s}:level${d}`)}if((levels[11]||0)-(levels[1]||0)<5)result.failures.push(`${s}:range-too-small`);if((levels[11]||0)<9)result.failures.push(`${s}:level11-too-low`);result.subjects[s]={rows:banks[s].length,means:levels};}
...
if(result.english[11].vocabMean-result.english[1].vocabMean<4)result.failures.push('english:vocab-range');
if(result.english[11].vocabMean<9)result.failures.push('english:vocab-level11-too-low');
...
const unitCases={japanese:'classical',math:'algebra',science:'chemistry',social:'history'};for(const [s,u] of Object.entries(unitCases)){const means={};for(const d of [2,10]){ctx.state.ui.subjectDifficulty=d;const cfg={subject:s,length:'standard',unitsBySubject:{[s]:[u]}},q=E.exactUnitQueue(cfg)||[];means[d]=+mean(q.map(x=>x.actualDifficulty)).toFixed(2);if(q.length<3||q.some(x=>x.examUnit!==u))result.failures.push(`${s}:unit-${d}`)}if(means[10]-means[2]<3)result.failures.push(`${s}:unit-range`);result.unitPractice[s]={unit:u,means};}
...
console.log('DIFFICULTY_5SUBJECT_AUDIT '+JSON.stringify(result));
