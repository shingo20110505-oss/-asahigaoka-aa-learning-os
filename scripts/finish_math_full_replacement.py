...
old='''    const mathSets = [1401,2202,3503,4804].map(seed => window.AAMathEngine.buildSet(seed, 2));
...
'''
new='''    const mathSets = [1401,2202,3503,4804].map(seed => window.AAMathEngine.buildSet(seed, 2));
    const math = mathSets.flat();
...
d=Path('.github/workflows/deploy-pages.yml')
s=d.read_text(encoding='utf-8')
patches=[
('''            math-exam/engine.js \\
            sw.js; do''',
 '''            math-exam/engine.js \\
            math-exam/adapter.js \\
            sw.js; do'''),
('''          node scripts/validate-math.mjs
''',
 '''          node scripts/validate-math.mjs
          node tests/math-full-replacement.mjs
'''),
('''          grep -q "url('ai-reading-v1.js')" sw.js
''',
 '''          grep -q "url('ai-reading-v1.js')" sw.js
          grep -q "url('math-exam/adapter.js')" sw.js
          grep -q 'math-exam/adapter.js?version=1.1.0' index.html
'''),
('''          grep -q "url('ai-reading-v1.js')" /tmp/sw.js
''',
 '''          grep -q "url('ai-reading-v1.js')" /tmp/sw.js
          grep -q "url('math-exam/adapter.js')" /tmp/sw.js
'''),
('''          grep -q 'data-aa-quality-ci="PASS"' /tmp/quality-runtime.html
''',
 '''          grep -q 'data-aa-quality-ci="PASS"' /tmp/quality-runtime.html
          grep -q 'data-aa-math-full-replacement="PASS"' /tmp/quality-runtime.html
'''),
('''          grep -q '愛知県型数学・応用検算' /tmp/quality-runtime.html
''',
 '''          grep -q '愛知県型数学・応用検算' /tmp/quality-runtime.html
          grep -q '通常演習も検証済みエンジンへ完全置換' /tmp/quality-runtime.html
'''),
('''          MATH_ENGINE_SHA256=$(sha256sum math-exam/engine.js | awk '{print $1}')
''',
 '''          MATH_ENGINE_SHA256=$(sha256sum math-exam/engine.js | awk '{print $1}')
          MATH_ADAPTER_SHA256=$(sha256sum math-exam/adapter.js | awk '{print $1}')
'''),
('''          math_engine_sha256=${MATH_ENGINE_SHA256}
''',
 '''          math_engine_sha256=${MATH_ENGINE_SHA256}
          math_adapter_sha256=${MATH_ADAPTER_SHA256}
'''),
('''          checks=japanese-exam-public,public-release,review-v2,review-bank-exact-hash,mobile-layout-exact-hash,quality-repair-exact-hash,ai-reading-exact-hash,math-engine-exact-hash,math-runtime,quality-browser-runtime,vocab-mobile-runtime,login-image,login-voice,service-worker,diagnostics
''',
 '''          checks=japanese-exam-public,public-release,review-v2,review-bank-exact-hash,mobile-layout-exact-hash,quality-repair-exact-hash,ai-reading-exact-hash,math-engine-exact-hash,math-adapter-exact-hash,math-full-replacement-runtime,math-runtime,quality-browser-runtime,vocab-mobile-runtime,login-image,login-voice,service-worker,diagnostics
'''),
]
for old,new in patches:
    if new in s:
        continue
    if old not in s:
        raise AssertionError(f'deploy marker missing: {old[:120]!r}')
    s=s.replace(old,new,1)
