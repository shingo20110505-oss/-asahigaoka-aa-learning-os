...
function extractJSONArray(source,marker){
 const p=source.indexOf(marker);if(p<0)throw new Error(marker+' not found');let i=source.indexOf('[',p),depth=0,str=false,esc=false;
 for(let j=i;j<source.length;j++){
  const c=source[j];if(str){if(esc)esc=false;else if(c==='\\')esc=true;else if(c==='"')str=false;continue}
  if(c==='"'){str=true;continue}if(c==='[')depth++;else if(c===']'){depth--;if(depth===0)return JSON.parse(source.slice(i,j+1))}
 }
