...
for(const s of subs){
  const rows=C[s]||[],ids=new Set(),dups=[],bad=[],promptAnswers=new Map(),exact=new Set(),exactDups=[],conflicts=[],unitCounts={},difficultyCounts={},templateCounts={};
  for(const r of rows){
    ...
    if(exact.has(key))exactDups.push(String(r[0]));else exact.add(key);
    if(prompt){const prev=promptAnswers.get(prompt);if(prev&&prev.answer!==answer)conflicts.push({id:String(r[0]),otherId:prev.id});else if(!prev)promptAnswers.set(prompt,{answer,id:String(r[0])});}
  ...
  result.deepAudit[s]={exactDuplicateQuestionAnswer:exactDups.length,conflictingAnswersForSamePrompt:conflicts.length,unitCounts,difficultyCounts,repeatedTemplates};
  if(dups.length||bad.length)fail.push(s+':invalid');
  if(conflicts.length)fail.push(s+':conflictingAnswersForSamePrompt='+conflicts.length);
}
...
for(let i=0;i<vocab.length;i++){
  const x=vocab[i],key=norm(typeof x==='string'?x:(x?.word??x?.term??x?.en??JSON.stringify(x)));
  if(key&&vocabSeen.has(key))vocabDup.push(i);else if(key)vocabSeen.add(key);
}
...
result.deepAudit.note='Exact duplicate and conflicting-answer checks are fail-safe checks. Repeated template counts are diagnostic warnings only because repeated forms can be pedagogically intentional.';
