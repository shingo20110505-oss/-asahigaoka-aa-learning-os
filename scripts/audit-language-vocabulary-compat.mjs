...
function hasJapanese(v){ return /[\u3040-\u30ff\u3400-\u9fff]/.test(String(v||'')); }
...
const enMeta = enSandbox.window.RISE_ENGLISH_VOCAB_SUPPLEMENT || {};
const enAdded = enSandbox.DATA.vocab.slice(1);
...
const legacyAfter = enSandbox.DATA.vocab[0];
...
const jaMeta = jaSandbox.window.RISE_JAPANESE_VOCAB_SUPPLEMENT || {};
const jaRows = jaSandbox.window.AA_JUKUGO_ADVANCED;
...
const rawRows = jaLines.map(JSON.parse);
const rawKeys = rawRows.map(x=>`${String(x.term||'').trim()}|${String(x.reading||'').trim()}`);
...
for(const key of ['kokugoChronologiaStateV2','aa_kokugo_vocab_wrong_queue_v1','aa_kokugo_vocab_full15000_cycle_v1']){
  check(`Japanese native state key preserved: ${key}`, rankQuiz.includes(key));
}
...
const enPos = loader.indexOf('english-vocabulary-supplement-v1.js');
const glossPos = loader.indexOf('v23-english-gloss-vocab.js');
const mainPos = loader.indexOf('v23-english-main.js');
...
const jukugoExt = read('kokugo-chronologia/jukugo-extension.js');
const adv5Pos = jukugoExt.indexOf('jukugo-bank-advanced-5.js');
const supPos = jukugoExt.indexOf('jukugo-bank-supplement-v1.js');
check('Japanese supplement loads after existing advanced banks', supPos > adv5Pos, `${adv5Pos}<${supPos}`);

const sw = read('sw.js');
check('PWA precaches English supplement', sw.includes("url('english-vocabulary-supplement-v1.js')"));
check('PWA precaches Japanese supplement', sw.includes("url('kokugo-chronologia/jukugo-bank-supplement-v1.js')"));
check('PWA cache version includes language vocabulary marker', sw.includes('language-vocab-1.0.0'));

const failures = checks.filter(x=>!x.ok);
console.log(JSON.stringify({
  version:'1.0.0',
  english:{candidates:enMeta.candidateCount, addedToEmptyBaseline:enMeta.added},
  japanese:{candidates:jaMeta.candidateCount, addedToEmptyAdvanced:jaMeta.added},
  checks,
  failures
}, null, 2));
