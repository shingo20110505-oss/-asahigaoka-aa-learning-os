...
function installBrowser(api){
 ...
 const apply=()=>{const sec=document.querySelector('#kkSection');if(!sec)return false;const h=sec.querySelector('.kk-head h2');if(h)h.textContent='古文・漢文 学習カード1700';const note=sec.querySelector('.kk-head .note');if(note)note.textContent=`古文・漢文の学習カード1,700件。原語を削除せず、基礎・重要語義・文脈語義・文法識別・和歌読解・文学史確認を学習フェーズとして区別します。実質概念 ${api.uniqueConcepts.toLocaleString()}件／再演習カード ${api.repeatCards.toLocaleString()}件。`;let box=sec.querySelector('#kkNormalizationSummary');if(!box){box=document.createElement('div');box.id='kkNormalizationSummary';box.className='note';box.style.cssText='margin:8px 0 10px;padding:9px 11px;border:1px solid var(--line);border-radius:10px;background:#f8fafc;line-height:1.65';sec.querySelector('.kk-summary')?.after(box)}box.textContent=['古語','敬語','文法','和歌','表現技法','古典常識','漢文'].map(k=>`${k} ${api.counts[k]||0}`).join(' ・ ');for(const row of sec.querySelectorAll('.kk-row')){const cell=row.querySelector('.kk-rank');if(!cell)continue;const n=Number((cell.textContent.match(/\d+/)||[])[0]);const meta=api.byRank.get(n);if(!meta)continue;const tier=cell.querySelector('.kk-tier');if(tier)tier.textContent=meta.displayBand;const cat=row.querySelector('.kk-cat');if(cat){cat.textContent=meta.domain;cat.title=meta.phase+(meta.isRepeat?`／基礎カード #${meta.variantOf} の再演習`:'')}}return true};
