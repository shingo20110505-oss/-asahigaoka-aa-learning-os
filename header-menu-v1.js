...
window.__AA_HEADER_MENU_V1__={version:'1.0.2'};
...
let raf=0,lastFocus=null,lastTouchActionAt=0;
...
function injectStyle(){
 ...
 #${OVERLAY_ID}{position:fixed;inset:0;z-index:2147483000;background:rgba(7,12,22,.48);opacity:0;pointer-events:none;transition:opacity .18s ease;isolation:isolate}
 #${OVERLAY_ID}.open{opacity:1;pointer-events:auto!important}
 #${PANEL_ID}{position:absolute;z-index:1;top:0;right:0;height:100%;width:min(360px,88vw);background:var(--card,#fff);color:var(--ink,#172033);box-shadow:-18px 0 48px rgba(0,0,0,.24);transform:translateX(104%);transition:transform .2s ease;overflow:auto;-webkit-overflow-scrolling:touch;overscroll-behavior:contain;pointer-events:auto!important;padding:calc(14px + env(safe-area-inset-top,0px)) 14px calc(18px + env(safe-area-inset-bottom,0px));font-family:-apple-system,BlinkMacSystemFont,"Hiragino Sans","Yu Gothic UI","Yu Gothic",sans-serif}
 ...
 .aaMenuHead strong{font-size:16px}.aaMenuClose{position:relative;z-index:2;border:1px solid var(--line,#d9e0ea);background:transparent;color:inherit;border-radius:10px;min-width:40px;min-height:40px;font-size:20px;font-weight:900;cursor:pointer;pointer-events:auto!important;touch-action:manipulation}
 .aaMenuList{display:grid;gap:8px;padding:14px 0;pointer-events:auto!important}.aaMenuItem{position:relative;z-index:2;width:100%;display:grid;grid-template-columns:34px 1fr auto;align-items:center;gap:10px;text-align:left;border:1px solid var(--line,#d9e0ea);background:var(--card,#fff);color:var(--ink,#172033);border-radius:13px;padding:11px 12px;min-height:52px;text-decoration:none;font:inherit;cursor:pointer;pointer-events:auto!important;touch-action:manipulation;-webkit-user-select:none;user-select:none;-webkit-tap-highlight-color:rgba(36,88,211,.12)}
 .aaMenuItem>*{pointer-events:none}.aaMenuItem:active{transform:translateY(1px);background:color-mix(in srgb,var(--card,#fff) 88%,var(--blue2,#eaf0ff))}.aaMenuIcon{font-size:19px;text-align:center}.aaMenuText{min-width:0}.aaMenuText b{display:block;font-size:13px}.aaMenuText small{display:block;margin-top:2px;color:var(--sub,#667085);font-size:10px;line-height:1.35;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.aaMenuArrow{color:var(--muted,#98a2b3);font-weight:900}
...
function renderItems(){
 ...
 list.innerHTML=items.map(x=>`<button type="button" class="aaMenuItem" data-aa-menu-key="${x.key}" aria-label="${esc(x.label)}"><span class="aaMenuIcon">${x.icon}</span><span class="aaMenuText"><b>${esc(x.label)}</b><small>${esc(x.sub)}</small></span><span class="aaMenuArrow">›</span></button>`).join('');
}
...
function menuTarget(target){
 ...
 return target.closest('#'+TOGGLE_ID+',.aaMenuClose,[data-aa-menu-key],#'+OVERLAY_ID);
}
function handleMenuEvent(e,isTouch=false){
 const target=menuTarget(e.target);if(!target)return false;
 if(target.id===OVERLAY_ID&&e.target!==target)return false;
 if(!isTouch&&Date.now()-lastTouchActionAt<700){e.preventDefault();e.stopPropagation();return true}
 if(isTouch)lastTouchActionAt=Date.now();
 e.preventDefault();e.stopPropagation();
 if(target.id===TOGGLE_ID){openMenu();return true}
 if(target.classList.contains('aaMenuClose')){closeMenu();return true}
 if(target.hasAttribute('data-aa-menu-key')){activate(target.dataset.aaMenuKey);return true}
 if(target.id===OVERLAY_ID){closeMenu();return true}
 return false;
}
...
function bind(){
 ...
 if('PointerEvent' in window){
  document.addEventListener('pointerup',e=>{if(e.pointerType==='touch'||e.pointerType==='pen')handleMenuEvent(e,true)},true);
 }else{
  document.addEventListener('touchend',e=>handleMenuEvent(e,true),{capture:true,passive:false});
 }
