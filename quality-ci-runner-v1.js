(() => {
  ...
  function riseVisualReady() {
    ...
    return root.dataset.riseSkin === 'v4' && root.dataset.riseUi === '4' && root.dataset.riseStructure === 'optimized-4' && brand === 'Rise' && !!home && nav.length === 4 && labels.join('/') === 'ホーム/入試/学習/復習' && window.__RISE_INFORMATION_ARCHITECTURE_V1__?.version === '1.0.2';
  }
  ...
  async function audit() {
    ...
    add('Rise・最終4領域ナビ', riseRoot.dataset.riseStructure === 'optimized-4' && riseNav.length === 4 && riseNavLabels.join('/') === 'ホーム/入試/学習/復習' && window.__RISE_INFORMATION_ARCHITECTURE_V1__?.version === '1.0.2', `structure=${riseRoot.dataset.riseStructure || '-'} / nav=${riseNavLabels.join('/') || '-'} / ia=${window.__RISE_INFORMATION_ARCHITECTURE_V1__?.version || '-'} / visual-productionで別検証`, false);
    let clozeBad = 0;
    ...
    const aiExamPoolFirst = aiExam?.version === '1.3.0' && aiExam?.endpointPath === '/v1/exam' && aiExam?.poolPath === './verified-question-pool-v1.json' && aiExam?.requiresFrontendToken === false && aiExam?.usesLegacyFallback === false && aiExam?.liveGenerationOnUserAction === false && aiExam?.deliveryMode === 'verified-pool-first' && aiExam?.cacheFallback === 'verified-ai-only' && aiExamSubjects === 'japanese/math/science/social';
    add('AI入試4教科・検証済みプール配信', aiExamPoolFirst, `version=${aiExam?.version || '-'} / route=${aiExam?.endpointPath || '-'} / pool=${aiExam?.poolPath || '-'} / subjects=${aiExamSubjects || '-'} / liveGeneration=${String(aiExam?.liveGenerationOnUserAction)} / delivery=${aiExam?.deliveryMode || '-'} / frontendToken=${String(aiExam?.requiresFrontendToken)} / legacyFallback=${String(aiExam?.usesLegacyFallback)} / cache=${aiExam?.cacheFallback || '-'}`);
