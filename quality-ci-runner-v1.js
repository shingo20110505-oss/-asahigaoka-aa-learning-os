(() => {
  ...
  async function audit() {
    ...
    add('AI入試3教科・API専用経路', aiExam?.version === '1.1.0' && aiExam?.endpointPath === '/v1/exam' && aiExam?.requiresFrontendToken === false && aiExam?.usesLegacyFallback === false && aiExam?.cacheFallback === 'verified-ai-only' && aiExamSubjects === 'math/science/social', `route=${aiExam?.endpointPath || '-'} / subjects=${aiExamSubjects || '-'} / frontendToken=${String(aiExam?.requiresFrontendToken)} / legacyFallback=${String(aiExam?.usesLegacyFallback)} / cache=${aiExam?.cacheFallback || '-'}`);
