(() => {
  ...
  async function audit() {
    ...
    const aiExamPoolFirst = aiExam?.version === '1.2.0' && aiExam?.endpointPath === '/v1/exam' && aiExam?.poolPath === './verified-question-pool-v1.json' && aiExam?.requiresFrontendToken === false && aiExam?.usesLegacyFallback === false && aiExam?.liveGenerationOnUserAction === false && aiExam?.deliveryMode === 'verified-pool-first' && aiExam?.cacheFallback === 'verified-ai-only' && aiExamSubjects === 'math/science/social';
    add('AI入試3教科・検証済みプール配信', aiExamPoolFirst, `version=${aiExam?.version || '-'} / route=${aiExam?.endpointPath || '-'} / pool=${aiExam?.poolPath || '-'} / subjects=${aiExamSubjects || '-'} / liveGeneration=${String(aiExam?.liveGenerationOnUserAction)} / delivery=${aiExam?.deliveryMode || '-'} / frontendToken=${String(aiExam?.requiresFrontendToken)} / legacyFallback=${String(aiExam?.usesLegacyFallback)} / cache=${aiExam?.cacheFallback || '-'}`);
