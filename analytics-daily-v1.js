(()=>{'use strict';
if(window.__AA_DAILY_ANALYTICS_V2__)return;
...
let lastRaw='',lastSessionSig='',lastRenderSig='',showAll=false,renderTimer=0;
...
function ledger(){const x=readLocal(LEDGER_KEY,{});return Object.assign({version:2,sessions:{}},plainObj(x)?x:{})}
...
function dayKey(t=Date.now()){const d=new Date(t),y=d.getFullYear(),m=String(d.getMonth()+1).padStart(2,'0'),dd=String(d.getDate()).padStart(2,'0');return `${y}-${m}-${dd}`}
function addDays(k,n){const [y,m,d]=String(k||'').split('-').map(Number),x=new Date(y,m-1,d);if(!Number.isFinite(x.getTime()))return dayKey();x.setDate(x.getDate()+n);return dayKey(x.getTime())}
...
function fmtClock(t){if(!t)return'--';const d=new Date(t);if(!Number.isFinite(d.getTime()))return'--';return `${String(d.getHours()).padStart(2,'0')}:${String(d.getMinutes()).padStart(2,'0')}`}
...
function rebuiltDaysFromAttempts(attempts){
 ...
 for(const a of (Array.isArray(attempts)?attempts:[])){
  if(!Number.isFinite(Number(a?.timestamp)))continue;
  const k=dayKey(Number(a.timestamp)),d=out[k]||(out[k]={attempts:0,correct:0,ms:0});
  d.attempts++;if(a.correct)d.correct++;d.ms+=finite0(a.responseMs);
 }
 return out;
}
function repairDayStats(s){
 if(!plainObj(s))return false;
 if(!plainObj(s.stats))s.stats={};
 const original=plainObj(s.stats.days)?s.stats.days:{};
 const rebuilt=rebuiltDaysFromAttempts(s.attempts);
 const fixed={};let changed=!plainObj(s.stats.days);
 for(const [k,v] of Object.entries(original)){
  if(!plainObj(v)){
   fixed[k]=rebuilt[k]||{attempts:0,correct:0,ms:0};changed=true;continue;
  }
  const nv={...v,attempts:finite0(v.attempts),correct:finite0(v.correct),ms:finite0(v.ms)};
  if(nv.attempts!==v.attempts||nv.correct!==v.correct||nv.ms!==v.ms)changed=true;
  fixed[k]=nv;
 }
 for(const [k,v] of Object.entries(rebuilt))if(!(k in fixed)){fixed[k]=v;changed=true}
 if(changed)s.stats.days=fixed;
 return changed;
}
function repairAnalyticsState(){
 let changed=false;
 try{
  if(typeof state!=='undefined'&&plainObj(state)){
   changed=repairDayStats(state)||changed;
   if(changed){if(typeof save==='function')save();else try{localStorage.setItem(MAIN_KEY,JSON.stringify(state))}catch(_){}}
   return changed;
  }
 }catch(e){console.warn('analytics in-memory repair failed',e)}
 try{
  const raw=localStorage.getItem(MAIN_KEY);if(!raw)return false;
  const s=safeJSON(raw,null);if(!plainObj(s))return false;
  changed=repairDayStats(s)||changed;
  if(changed)localStorage.setItem(MAIN_KEY,JSON.stringify(s));
 }catch(e){console.warn('analytics storage repair failed',e)}
 return changed;
}
function installBaseAnalyticsGuard(){
 try{
  if(typeof analyticsHTML!=='function'||analyticsHTML.__aaDailyGuard)return;
  const original=analyticsHTML;
  const guarded=function(){
   repairAnalyticsState();
   try{return original()}
   catch(e){
    console.error('Analytics render recovered',e);
    const attempts=(typeof state!=='undefined'&&Array.isArray(state?.attempts))?state.attempts.length:0;
    const body=`<section class="card"><div class="eyebrow">ANALYTICS RECOVERY</div><h2 class="h2">学習分析</h2><div class="notice"><b>分析データを安全に復旧しました。</b><br>学習履歴は削除していません。もう一度「分析」を開くと通常表示を再試行します。</div><div class="sp12"></div><div class="metricRow"><div class="metric"><b>${attempts}</b><span>保存済み回答</span></div></div></section>`;
    return typeof layout==='function'?layout(body):body;
   }
  };
  guarded.__aaDailyGuard=true;guarded.__original=original;analyticsHTML=guarded;
 }catch(e){console.warn('analytics guard install failed',e)}
}
...
function captureSession(s){
 const ss=s?.session;if(!ss?.id||!ss?.startedAt)return;
 const sig=[ss.id,ss.active,ss.endedAt,ss.accumulatedMs,ss.abandoned,ss.index,ss.clockPaused].join('|');if(sig===lastSessionSig)return;lastSessionSig=sig;if(!ss.endedAt&&!ss.clockPaused)return;
 const l=ledger(),old=l.sessions[ss.id]||{};
 const rec={id:ss.id,startedAt:Number(ss.startedAt)||Date.now(),endedAt:Number(ss.endedAt)||null,ms:finite0(ss.accumulatedMs),mode:ss.mode||'',kind:ss.kind||'',subject:ss.subject||'',completed:!!ss.endedAt&&!ss.abandoned&&!ss.active,abandoned:!!ss.abandoned,updatedAt:Date.now(),streakRecorded:!!old.streakRecorded};
 ...
 if(rec.completed&&!rec.streakRecorded&&dayKey(rec.endedAt||Date.now())===dayKey()){try{if(window.Companion7?.recordStudyComplete){window.Companion7.recordStudyComplete();l.sessions[ss.id].streakRecorded=true}}catch(_){}}
 saveLedger(l);
}
...
function buildDays(s){
 ...
 for(const a of (Array.isArray(s?.attempts)?s.attempts:[])){
  if(!Number.isFinite(Number(a?.timestamp)))continue;const t=Number(a.timestamp),k=dayKey(t),d=get(k);d.attempts++;if(a.correct)d.correct++;d.responseMs+=finite0(a.responseMs);d.firstAt=d.firstAt==null?t:Math.min(d.firstAt,t);d.lastAt=d.lastAt==null?t:Math.max(d.lastAt,t+finite0(a.responseMs));const sub=inferSubject(a);d.subjects[sub]=(d.subjects[sub]||0)+1;if(!a.correct){const e=String(a.errorType||'未分類');d.errors[e]=(d.errors[e]||0)+1}if(a.sessionId){d.sessionIds.add(a.sessionId);const id=`${k}|${a.sessionId}`,g=bySession[id]||(bySession[id]={day:k,id:a.sessionId,min:t,max:t,sum:0});g.min=Math.min(g.min,t);g.max=Math.max(g.max,t+finite0(a.responseMs));g.sum+=finite0(a.responseMs)}
 }
 for(const x of (Array.isArray(s?.stats?.readingPace)?s.stats.readingPace:[])){if(!Number.isFinite(Number(x?.at)))continue;const d=get(dayKey(Number(x.at)));if(Number.isFinite(Number(x.wpm)))d.readingWpm.push(Number(x.wpm));d.readings++}
 for(const x of (Array.isArray(s?.stats?.lexicalSessions)?s.stats.lexicalSessions:[])){if(!Number.isFinite(Number(x?.at)))continue;const d=get(dayKey(Number(x.at)));if(Number.isFinite(Number(x.coverage)))d.coverage.push(Number(x.coverage))}
 ...
 const live=s?.session;if(live?.active&&live.startedAt){const d=get(dayKey(Number(live.startedAt)));let ms=finite0(live.accumulatedMs);if(!live.clockPaused&&live.lastActiveAt)ms+=Math.max(0,Date.now()-Number(live.lastActiveAt));if(!d.exactIds.has(live.id))d.estimatedMs+=ms;if(live.id)d.sessionIds.add(live.id)}
 for(const d of Object.values(map)){if(d.exactMs>0){d.studyMs=d.exactMs+d.estimatedMs;d.timeQuality=d.estimatedMs>0?'実測＋推定':'実測'}else if(d.estimatedMs>0){d.studyMs=d.estimatedMs;d.timeQuality='推定'}else{d.studyMs=d.responseMs;d.timeQuality=d.responseMs>0?'回答時間':'なし'}}
 return map;
}
...
function renderDaily(){
 ...
 css();const s=mainState(),days=buildDays(s),today=dayKey(),keys=[];for(let i=0;i<30;i++)keys.push(addDays(today,-i));const active30=keys.filter(k=>{const d=days[k];return d&&(d.attempts>0||d.studyMs>0)}).length,last7=keys.slice(0,7).map(k=>days[k]||baseDay(k)),ms7=last7.reduce((a,d)=>a+(d.studyMs||0),0),att7=last7.reduce((a,d)=>a+d.attempts,0),cor7=last7.reduce((a,d)=>a+d.correct,0),st=streak(),visible=showAll?keys:keys.slice(0,14);
 const old=[...main.querySelectorAll('section.card')].find(x=>x.querySelector('h3')?.textContent?.includes('直近学習日'));if(old&&old.style.display!=='none')old.style.display='none';
 let card=document.getElementById('aaDailyAnalyticsCard');if(!card){card=document.createElement('section');card.className='card';card.id='aaDailyAnalyticsCard';if(old)old.insertAdjacentElement('beforebegin',card);else main.appendChild(card)}
 const html=`<div class="eyebrow">DAILY RECORD</div><h3 class="h3">日々の学習記録</h3><div class="aaSummary"><div><b>${finite0(st.current)}日</b><span>現在の連続学習</span></div><div><b>${active30}/30</b><span>直近30日の学習日</span></div><div><b>${fmtMs(ms7)}</b><span>直近7日学習時間</span></div><div><b>${att7?Math.round(cor7/att7*100)+'%':'--'}</b><span>直近7日正答率</span></div></div><div class="tiny">30日継続マップ</div><div class="aaHeat">${[...keys].reverse().map(k=>{const d=days[k],on=d&&(d.attempts>0||d.studyMs>0);return `<i class="${on?'on':''}" title="${k}"></i>`}).join('')}</div>${visible.map(k=>dayCard(days[k]||baseDay(k),today)).join('')}<div class="actions" style="margin-top:10px"><button class="btn ghost sm" type="button" data-aa-day-more>${showAll?'14日表示に戻す':'30日分を見る'}</button></div><div class="tiny" style="margin-top:10px">壊れた旧形式の日別統計は回答履歴から安全に復元します。回答履歴・習熟度・復習進捗は削除しません。</div>`;
 const sig=[showAll,localStorage.getItem(LEDGER_KEY)||'',localStorage.getItem(STREAK_KEY)||'',lastRaw,today].join('|');if(sig!==lastRenderSig){lastRenderSig=sig;card.innerHTML=html}
}
function scheduleRender(delay=0){clearTimeout(renderTimer);renderTimer=setTimeout(renderDaily,delay)}
function sync(){let raw='';try{raw=localStorage.getItem(MAIN_KEY)||''}catch(_){}if(raw!==lastRaw){lastRaw=raw;const s=safeJSON(raw,{});repairDayStats(s);captureSession(s);scheduleRender(0)}}
...
function start(){
 css();const repaired=repairAnalyticsState();installBaseAnalyticsGuard();sync();renderDaily();
 if(repaired&&typeof state!=='undefined'&&state?.route==='analytics'&&typeof render==='function')setTimeout(()=>{try{render()}catch(e){console.warn('analytics recovery rerender failed',e)}},0);
 setInterval(sync,600);setInterval(()=>scheduleRender(0),2500);
}
